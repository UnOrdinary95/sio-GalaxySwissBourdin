 ...
 * Inclut les informations du médecin associé si type=visiteur,
 * ou les informations du visiteur associé si type=medecin.
 ...
 * @returns {Promise<{ items: (RapportWithMedecin | RapportWithVisiteur)[]; total: number }>} Liste paginée des rapports avec infos associées et total
...
export const findManyRapportsPaginated = async (
    ...
    items: (RapportWithMedecin | RapportWithVisiteur)[];
    total: number;
}> => {
    try {
        ...
        if (type === 'visiteur') {
            // Récupération des rapports avec infos médecin
            ...
            // Récupération des rapports avec infos visiteur
            const itemsResult = await pool.query(
                `SELECT r.id, r.date, r.motif, r.bilan, r.idvisiteur, r.idmedecin,
                        v.id as visiteur_id, v.nom as visiteur_nom, v.prenom as visiteur_prenom
                 FROM rapport r
